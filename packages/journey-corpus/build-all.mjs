...
//   node build-all.mjs --check   same, then fail if regeneration changed any
//                                generated bytes (used in CI and dirty worktrees)
//
...
const GENERATED = [
  ...
  "evaluation/taxonomy-audit.json",
  "evaluation/blocker-labeling-packet.json",
  "evaluation/blocker-evaluation-status.json",
  "evaluation/association-analysis-plan.json",
  "evaluation/product-validation-protocols.json",
];
const generatedBefore = check
  ? new Map(
      GENERATED.map((file) => {
...
run(`node scripts/build-corpus-health.mjs${check ? " --check" : ""}`);
run("node scripts/build-validation-foundations.mjs");
...
// 8. Deterministic generation gate. Comparing bytes before and after works in
// CI and in an intentional dirty worktree, while still failing on stale or
// missing generated artifacts.
if (check) {
  const changed = GENERATED.filter((file) => {
    const absolute = path.join(ROOT, file);
    const before = generatedBefore?.get(file) ?? null;
    const after = fs.existsSync(absolute) ? fs.readFileSync(absolute) : null;
    return before === null || after === null || !before.equals(after);
  });
  if (changed.length) fail(`generation changed stale or missing artifacts:\n${changed.join("\n")}`);
